...
- **房间列表**：`LIST_ROOMS(1013)` → `ROOMS_LIST(1014)`，摘要包含房间名、阶段、人数、地图尺寸
- **房间配置**：
  - 服务端启动时读取 `game_config.json` 作为默认值
  - 创建房间时可通过 payload 覆盖（支持 legacy 平铺字段，也支持 `payload.config` 结构化覆盖）
  - 服务端对配置执行强制边界截断（Clamp），保证上/下限安全；客户端在配置表格确认输入时会镜像同规则并回填矫正值

### 3.2 断线重连与超时踢出

- 服务端维护玩家断线标记与断线时间戳；在 `server.disconnect_grace_sec` 宽限期内，使用同 `session_id` 重连会复用原玩家进度
- 超过宽限期会清除该玩家在该房间内的进度并视为离开（人数减少）
- 客户端支持自动重连；冷启动续局需要在 CONNECT 入口提供 `session_id`（Resume ID）才会尝试自动回到上次房间
