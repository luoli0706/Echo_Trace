...
class Renderer:
    def __init__(self, screen):
        ...
        if not os.path.exists(icon_path):
             ...
             if not os.path.exists(icon_path):
                 icon_path = os.path.join("..", "assets", "icos") # if running from client folder
        ...
        def get_cjk_font(size):
            # Common CJK font names for Windows, Linux, macOS
            ...
            for name in font_names:
                ...
                if font_path:
                    try:
                        ...
                        continue
            return pygame.font.SysFont("arial", size) # Fallback

        self.font = get_cjk_font(FONT_SIZE)
        # HUD font also needs CJK support for texts like "操作: ..."
        self.hud_font = get_cjk_font(16)
        self.time_font = pygame.font.SysFont("consolas", 24) # Time is numbers, Consolas is fine
    ...
    def draw_connect(self):
        self.screen.fill(COLOR_BG)
        title = self.font.render(self.t("CONNECT_TITLE"), True, (0, 255, 255))
        self.screen.blit(title, title.get_rect(center=(WINDOW_WIDTH//2, 200)))
        
        input_rect = pygame.Rect(WINDOW_WIDTH//2 - 200, 300, 400, 40)
        ...
        hint = self.hud_font.render(self.t("ENTER_URL"), True, (150, 150, 150))
        self.screen.blit(hint, (WINDOW_WIDTH//2 - 150, 360))

    def draw_login(self):
        self.screen.fill(COLOR_BG)
        title = self.font.render(self.t("LOGIN_TITLE"), True, (0, 255, 255))
        self.screen.blit(title, title.get_rect(center=(WINDOW_WIDTH//2, 200)))
        ...
        surf = self.font.render(txt, True, (255, 255, 255))
        self.screen.blit(surf, (input_rect.x + 10, input_rect.y + 5))
        hint = self.hud_font.render(self.t("ENTER_NAME"), True, (150, 150, 150))
        self.screen.blit(hint, (WINDOW_WIDTH//2 - 150, 360))
        
        # Back Button (Red)
        r = pygame.Rect(WINDOW_WIDTH//2 - 60, 400, 120, 40)
        pygame.draw.rect(self.screen, (200, 50, 50), r, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), r, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (r.x+25, r.y+10))
        self.login_back_rect = r

    def draw_menu(self):
        self.screen.fill(COLOR_BG)
        title = self.font.render(self.t("MENU_TITLE"), True, (0, 255, 255))
        self.screen.blit(title, title.get_rect(center=(WINDOW_WIDTH//2, 100)))
        
        c_rect = pygame.Rect(WINDOW_WIDTH//2 - 150, 250, 300, 50)
        j_rect = pygame.Rect(WINDOW_WIDTH//2 - 150, 330, 300, 50)
        
        for r, txt in [(c_rect, self.t("BTN_CREATE")), (j_rect, self.t("BTN_JOIN"))]:
            pygame.draw.rect(self.screen, COLOR_BTN, r)
            pygame.draw.rect(self.screen, (0, 255, 255), r, 2)
            surf = self.font.render(txt, True, (255, 255, 255))
        ...
        # Back Button (Red)
        r = pygame.Rect(WINDOW_WIDTH//2 - 60, 450, 120, 40)
        pygame.draw.rect(self.screen, (200, 50, 50), r, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), r, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (r.x+25, r.y+10))
        self.menu_back_rect = r

    def draw_config(self):
        self.screen.fill(COLOR_BG)
        title = self.font.render(self.t("CONFIG_TITLE"), True, (0, 255, 255))
        self.screen.blit(title, title.get_rect(center=(WINDOW_WIDTH//2, 50)))
        
        y = 150
        labels = {
            "max_players": self.t("LBL_MAX_AGENTS"),
            "motors": self.t("LBL_MOTORS"),
            "p1_dur": self.t("LBL_SEARCH"),
            "p2_dur": self.t("LBL_CONFLICT")
        }
        for i, key in enumerate(self.config_keys):
            ...
            y += 50
        hint = self.hud_font.render(self.t("CONFIG_HINT"), True, (150, 150, 150))
        self.screen.blit(hint, (WINDOW_WIDTH//2 - 200, 500))
        
        # Back Button (Red)
        r = pygame.Rect(WINDOW_WIDTH//2 - 60, 550, 120, 40)
        pygame.draw.rect(self.screen, (200, 50, 50), r, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), r, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (r.x+25, r.y+10))
        self.config_back_rect = r
    ...
    def draw_pause_menu(self):
        ...
        title = self.font.render(self.t("PAUSE_TITLE"), True, (0, 255, 255))
        ...
        for key, rect in self.pause_rects.items():
            ...
            pygame.draw.rect(self.screen, (0, 255, 255), rect, 1, border_radius=5)
            surf = self.hud_font.render(lbls[key], True, (255, 255, 255))
    ...
    def draw_hud(self, state):
        ...
        for t in [f"HP: {state.my_hp:.0f}%", f"CASH: ${state.funds}", f"POS: {int(state.my_pos[0])},{int(state.my_pos[1])}"]:
            self.screen.blit(self.hud_font.render(t, True, COLOR_HUD_TEXT), (10, y))
            y += 20
        ...
        surf = self.font.render(f"{p_txt} | {int(state.time_left)}s", True, (255, 255, 0))
        self.screen.blit(surf, surf.get_rect(center=(WINDOW_WIDTH//2, 30)))
        
        ctrl_hint = self.hud_font.render(self.t("HUD_CONTROLS"), True, (150, 150, 150))
        self.screen.blit(ctrl_hint, (WINDOW_WIDTH - 300, WINDOW_HEIGHT - 30))

    def draw_minimap(self, state):
        pygame.draw.rect(self.screen, COLOR_RADAR_BG, self.radar_rect, border_radius=10)
        pygame.draw.rect(self.screen, COLOR_RADAR_BORDER, self.radar_rect, 2, border_radius=10)
        
        scale = 140.0 / 32.0 
        ox, oy = self.radar_rect.x + 5, self.radar_rect.y + 5
        ...
        sx, sy = state.my_pos[0] * scale, state.my_pos[1] * scale
        pygame.draw.circle(self.screen, COLOR_SELF, (int(ox+sx), int(oy+sy)), 3)

    def draw_bar(self, x, y, val, max_val, color):
        pygame.draw.rect(self.screen, (50,50,50), (x, y, GRID_SIZE, 4))
        pygame.draw.rect(self.screen, color, (x, y, GRID_SIZE * (val/max_val), 4))

    def draw_hp_bar(self, x, y, hp, max_hp):
        pct = max(0, min(1, hp/max_hp))
        pygame.draw.rect(self.screen, (100,0,0), (x, y, GRID_SIZE, 4))
        pygame.draw.rect(self.screen, (0,255,0), (x, y, GRID_SIZE * pct, 4))
    ...
    def draw_inventory(self, state):
        for i in range(6):
            r = pygame.Rect(300 + i*60, WINDOW_HEIGHT-70, 50, 50)
            pygame.draw.rect(self.screen, COLOR_INV_BG, r, border_radius=5)
            if i < len(state.my_inventory):
                item = state.my_inventory[i]
                name = item.get("name", item.get("id", "???"))
                self.screen.blit(self.hud_font.render(name[:3], True, (255,255,255)), (r.x+5, r.y+15))

    def draw_events(self, state):
        y = 100
        for e in state.events[-5:]:
            surf = self.hud_font.render(f"> {e['msg']}", True, (255,100,255))
    ...
    def draw_settings_menu(self):
        ...
        t = self.font.render(self.t("SETTINGS_TITLE"), True, (255,255,255))
        self.screen.blit(t, (self.settings_rect.x+20, self.settings_rect.y+20))
        
        # Dev Toggle
        pygame.draw.rect(self.screen, (0,255,0) if self.dev_mode else (100,100,100), self.dev_mode_rect, 2)
        txt = self.hud_font.render(f"{self.t('LBL_DEV_MODE')}: {'ON' if self.dev_mode else 'OFF'}", True, (255,255,255))
        self.screen.blit(txt, (self.dev_mode_rect.x+10, self.dev_mode_rect.y+5))
        
        # Lang Toggle
        pygame.draw.rect(self.screen, (0,255,255), self.lang_rect, 2)
        l_txt = self.hud_font.render(f"{self.t('LBL_LANG')}", True, (255,255,255))
        self.screen.blit(l_txt, (self.lang_rect.x+10, self.lang_rect.y+5))
        
        # Back (Red)
        pygame.draw.rect(self.screen, (200, 50, 50), self.back_btn_rect, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), self.back_btn_rect, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (self.back_btn_rect.x+25, self.back_btn_rect.y+10))

    def draw_help_menu(self):
        pygame.draw.rect(self.screen, COLOR_MENU_BG, self.help_rect, border_radius=10)
        pygame.draw.rect(self.screen, (0,255,255), self.help_rect, 2, border_radius=10)
        t = self.font.render(self.t("MANUAL_TITLE"), True, (0,255,255))
        self.screen.blit(t, (self.help_rect.x+20, self.help_rect.y+20))
        
        lines = i18n.get_list("MANUAL_LINES")
        for i, l in enumerate(lines):
            self.screen.blit(self.hud_font.render(l, True, (200,200,200)), (self.help_rect.x+30, self.help_rect.y+80+i*30))
    ...
    def draw_shop_menu(self, state):
        ...
        title = self.font.render(self.t("SHOP_TITLE"), True, (255,215,0))
        self.screen.blit(title, (self.shop_rect.x + 20, self.shop_rect.y + 20))
        
        funds = self.font.render(f"{self.t('SHOP_FUNDS')} ${state.funds}", True, (0, 255, 0))
        ...
        if state.phase == 1:
            items = [
                (self.t("ITEM_STUN"), "WPN_SHOCK", 100),
                (self.t("ITEM_MED"), "SURV_MEDKIT", 50),
                (self.t("ITEM_SCAN"), "RECON_RADAR", 150),
            ]
        elif state.phase == 2:
            items = [
                (self.t("ITEM_TASER"), "WPN_SHOCK_T2", 200),
                (self.t("ITEM_MED_PLUS"), "SURV_MEDKIT_T2", 100),
                (self.t("ITEM_SCAN_PRO"), "RECON_RADAR_T2", 300),
            ]
        elif state.phase >= 3:
            items = [
                (self.t("ITEM_VOLT"), "WPN_SHOCK_T3", 350),
                (self.t("ITEM_SCAN_GLOBAL"), "RECON_RADAR_T3", 500),
                (self.t("ITEM_MED_PLUS"), "SURV_MEDKIT_T2", 100),
            ]
        ...
        for name, pid, cost in items:
            ...
            txt = f"{name} - ${cost} [{items.index((name, pid, cost)) + 1}]"
            surf = self.hud_font.render(txt, True, color)
            self.screen.blit(surf, (self.shop_rect.x + 30, self.shop_rect.y + y_off))
            y_off += 40
            
        hint = self.hud_font.render(self.t("SHOP_HINT"), True, (150, 150, 150))
        self.screen.blit(hint, (self.shop_rect.x + 30, self.shop_rect.y + 350))
        
        # Back (Red)
        r = pygame.Rect(self.shop_rect.centerx - 60, self.shop_rect.bottom - 50, 120, 40)
        pygame.draw.rect(self.screen, (200, 50, 50), r, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), r, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (r.x+25, r.y+10))
        self.shop_back_rect = r
    ...
    def draw_lobby(self, state):
        ...
        # Back Button in Lobby (Red)
        r = pygame.Rect(WINDOW_WIDTH//2 - 60, 400, 120, 40)
        pygame.draw.rect(self.screen, (200, 50, 50), r, border_radius=5)
        pygame.draw.rect(self.screen, (255, 255, 255), r, 2, border_radius=5)
        self.screen.blit(self.hud_font.render("BACK [B]", True, (255,255,255)), (r.x+25, r.y+10))
        self.lobby_back_rect = r