...
* **核心逻辑详解：**
  ...
  * **物理碰撞优化:** 
    * 玩家的**物理碰撞半径**缩小为网格大小的 `0.5` 倍（或更小），以确保角色在视觉上不会“嵌入”墙壁，也不会被狭窄通道卡住。
    * 服务端每 `server.tick_rate_ms` 更新一次坐标，严格校验玩家是否穿墙。
...
* **三级 LOD 广播:**
  ...
  3. **全局层:** 同步撤离点、电机状态及**高价值物资点**的雷达脉冲。
...
### 2.5 高价值物资投放 (High-Value Supply Drops)

* **触发机制:** 每个游戏阶段（Phase 1/2/3）开始时，系统会在地图中刷新数个**高价值物资箱**。
* **生成逻辑:**
  * **中心化分布:** 刷新点倾向于所有玩家当前位置的**几何中心**（高风险区域），迫使玩家向中心聚集争夺。
  * **雷达信标:** 这些物资箱具有与电机相同的**雷达脉冲属性**，其位置会在所有玩家的雷达上持续显示，直到被拾取。
  * **物资箱特性:** 物资箱不会随时间消失，直到被玩家拾取。
* **奖励机制:**
  * **越级掉落:** 掉落物品的品质（Tier）基于**下一阶段**的掉落概率计算（例如Phase 1掉落Phase 2等级装备，Phase 2掉落Tier 3）。
  * **数量递增:** 包含 1~3 个道具。随着游戏阶段推进（Phase 1 -> 3），掉落数量的随机权重逐渐向 3 倾斜。
  * **资金奖励:** 拾取物资箱将直接给予玩家大量**资金 (Funds)**。

### 2.6 经济系统 (Economy)

* **资金 (Funds):** 玩家在单局游戏内的货币资源。
* **获取途径:** 
  * 拾取普通宝藏。
  * 开启高价值物资箱。
  * (可选) 击杀其他玩家。
* **UI表现:** 客户端 HUD 需实时显示当前拥有的资金数值。
